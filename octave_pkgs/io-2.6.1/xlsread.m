...
##
## @var{options}: after these "regular" input arguments a number of
## optional arguments can be supplied in any desired order, but just one
## of each optional argument type:
## @table @asis
## @item @var{interface} (character value)
## @var{Interface} (a three-character case-insensitive text string) can
...
## or exclude one or more interfaces.
## @end item
...
## etc.) and do the actual reading.  parsecell() is invoked to separate
## the numerical and text data from the raw output array. @*
## For each call to xlsread the interface must be started and the
## spreadsheet file read into memory.  When reading multiple ranges (in

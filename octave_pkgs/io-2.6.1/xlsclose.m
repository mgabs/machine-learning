    ...
    for ii=1:nargin-1
      ...
      elseif (! isempty (strfind (tolower (varargin{ii}), ".")))
        ...
        elseif (strcmp (xls.xtype, "JXL"))
          error ("xlsclose: JXL doesn't support changing filename, new filename \
ignored.\n");
  ...
  unwind_protect_cleanup
    if (xls.changed && xls.changed < 3)
      ...
      if (force)
        ...
        printf ("(File pointer preserved.\n Try saving again later, or with \
different file name, or as different file type.)\n");
      endif
    else
      xls = [];
    endif

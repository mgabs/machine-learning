...
function [] = test_spsh (numb = [])
  ...
  for ii = 1:numel (intfs)
    try
      ...
      if (! isempty (intf))
        printf ("\nInterface \"%s\" found.\n", upper (intf));
        rslts = [rslts ; io_testscript(intf, fname)];
        if (intfchk == 128)
  ...
  rslts = [rslts; io_testscript("OCT", "io-test.xlsx")];
  ## Test OCT interface for ods
  rslts = [rslts; io_testscript("OCT", "io-test.ods")];
  ## Test OCT interface for gnumeric
  rslts = [rslts; io_testscript("OCT", "io-test.gnumeric")];
